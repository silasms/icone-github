defmodule IconeIdentidade do
  ...
  def criar_pixel(%IconeIdentidade.Imagem{grid: grid} = imagem) do
    pixel_map = Enum.map grid, fn{_cor, index} ->
      h = rem(index, 5) * 50
      v = div(index, 5) * 50
      t_esquerda = {h, v}
      i_direita = {h+50, v+50}
  ...
  def remover_impar(%IconeIdentidade.Imagem{grid: grid} = imagem) do
    new_grid = Enum.filter(grid, fn {numero, _indice} -> rem(numero, 2) == 0 end)
